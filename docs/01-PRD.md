...
flowchart TD
    subgraph 教研负责人
        T1[创建课程体系] --> T2[配置评价标准模板]
        T2 --> T3[建立课程知识库]
        T9[查看全院教学报表] --> T10[制定课程优化方案]
    end

    subgraph 授课教师
        C1[引用评价标准模板] --> C2[创建实训任务]
        C2 --> C3[下发至授课班级]
        C3 --> C4[监控提交进度]
...
**阶段一：课程筹备**
教研负责人创建课程体系，配置标准化的评价指标与评分权重模板，建立课程对应的知识库（含教学大纲、参考规范、常见问题等）。教师引用模板快速完成实训任务的评价标准配置。
...
### 8.3.1 标准模板管理
- 创建评价标准模板（名称、适用课程、版本号）
- 定义评价维度与子项（如代码规范性、功能完成度、设计质量、文档完整性）
- 每个维度配置评分权重（百分比，系统校验总和为100%）
- 模板版本管理与历史回溯

### 8.3.2 评分规则配置
- 为每个评价维度下的检查项配置扣分规则
- 支持自动检测规则（正则匹配、文件存在性、目录结构等）
- 支持人工评判规则（需教师主观评估的维度）
- 规则优先级与互斥关系设置
...
| 教师 | 首页、课程管理、实训管理、评价标准（引用）、知识库、任务中心、成果提交（查看）、AI分析结果、教师审核、报表中心（班级） |
| 教研负责人 | 首页、课程管理、实训管理、评价标准（管理）、知识库、报表中心（全院）、教师审核（只读） |
...
- **布局**：实训任务列表 + 进度监控面板
- **功能区**：实训任务创建/编辑、评价标准模板选择、提交规则配置、任务下发、进度统计（已提交/未提交/逾期）
- **核心交互**：创建实训时引用评价标准模板，下发后实时监控提交进度，支持批量催交
- **业务闭环**：实训任务连接评价标准与学生提交，是业务流程的核心枢纽
...
- **功能区**：
  ...
  - 自动检测规则配置（正则、文件匹配等）
  - 模板版本管理
- **核心交互**：教研负责人创建标准模板供全院复用，教师在创建实训时一键引用
- **业务闭环**：标准化评价体系支撑全院实训质量统一管控
